...
const NavSearch = () => {
  ...
  // Local storage key for recent searches per locale
  const RECENT_KEY = `recent-searches-${locale}`
  ...
  const onSearchItemSelect = (item: Item) => {
    ...
    // Update recent searches (unique by url)
    setRecent(prev => {
  ...
  // Load recent searches for current locale
  useEffect(() => {
    try {
      ...
      {isBreakpointReached || settings.layout === 'horizontal' ? (
        ...
        <div className='flex items-center gap-2 cursor-pointer' onClick={() => setOpen(true)}>
          ...
          <div className='whitespace-nowrap select-none text-textDisabled'>
            {locale === 'fr' ? 'Rechercher ⌘K' : 'Search ⌘K'}
          </div>
        </div>
      )}
        ...
        <CommandList>
          {searchValue ? (
            ...
            <>
              {recent.length > 0 && (
                <CommandGroup
                  heading={(locale === 'fr' ? 'Recherches récentes' : 'Recent Searches').toUpperCase()}
                  className='text-xs'
                >
                  {recent.map((item, idx) => (
                    <SearchItem
